...
if (isset($_POST['ganti'])) {
  ...
  if (mysqli_num_rows($result)==0) {
    echo "<script>alert('Mohon Maaf, Username $username Tidak Ditemukan! :('); window.location='$redirect';</script>";
  }else {
    ...
    if ($paslam=='' || $pasnew=='' || $conpas=='') {
      ...
      if ($paslam <> $cek['password']) {
        ...
        if ($pasnew <> $conpas) {
          ...
          if ($sql) {
            echo "<script>alert('PASSWORD BARU BERHASIL DISIMPAN! :)'); window.location='$redirect';</script>";
          }else {
            echo "<script>alert('PASSWORD BARU GAGAL DISIMPAN! :('); window.location='$redirect';</script>";
          }
        }
      }
    }
